...
export class ObjectStorageService {
  ...
  // Gets the upload URL for an enrollment document.
  async getDocumentUploadURL(): Promise<string> {
    ...
    const fullPath = `${privateObjectDir}/enrollment-documents/${documentId}`;
  ...
  async getDocumentFile(objectPath: string): Promise<File> {
    if (!objectPath.startsWith("/objects/enrollment-documents/")) {
    ...
    const documentPath = `${documentDir}enrollment-documents/${documentId}`;
  ...
  normalizeDocumentPath(rawPath: string): string {
    ...
    if (!rawObjectPath.startsWith(`${documentDir}enrollment-documents/`)) {
      return rawObjectPath;
    }
  
    // Extract the document ID from the path
    const documentId = rawObjectPath.slice(`${documentDir}enrollment-documents/`.length);
    return `/objects/enrollment-documents/${documentId}`;
  }
}
